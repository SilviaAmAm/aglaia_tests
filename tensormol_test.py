...
radial_embedding, pair_indices_rad, pair_elements = tf_symmetry_functions_radial_grid(xyzs, Zs, radial_cutoff, radial_rs, eta)
angular_embedding, triples_indices, triples_element, sorted_triples_element_pairs = tf_symmetry_function_angular_grid(xyzs, Zs, angular_cutoff, angular_rs, theta_s, zeta, eta)

pair_element_indices 	 = tf.cast(tf.where(tf.equal(tf.expand_dims(pair_elements[:,1], axis=-1), tf.expand_dims(elements, axis=0))), tf.int32)[:,1]
triples_elements_indices = tf.cast(tf.where(tf.reduce_all(tf.equal(tf.expand_dims(sorted_triples_element_pairs, axis=-2), element_pairs), axis=-1)), tf.int32)[:,1]
...
someting = sess.run(radial_atom_embeddings)
print(np.sort(someting))

# something2 = sess.run(sorted_triples_element_pairs)
# print(something2)